...
def resample_points(points: list[tuple[float, float]], resolution_m: float) -> list[tuple[float, float]]:
    resolution_m = float(resolution_m)
    if resolution_m <= 0.0 or len(points) < 2:
        return points
    ...
    for next_x, next_y in points[1:]:
        seg_dx = next_x - prev_x
        seg_dy = next_y - prev_y
        ...
        while distance_since_sample + remaining >= resolution_m:
            step = resolution_m - distance_since_sample
            cursor_x += unit_x * step
            cursor_y += unit_y * step
            resampled.append((cursor_x, cursor_y))
            remaining -= step
            distance_since_sample = 0.0
...
def main() -> None:
    ...
    default_link_set = "MORAI-ADModule/mgeo/lib/mgeo_data/R_KR_PG_K-City/link_set.json"
    parser.add_argument(
        "--link-set",
        default=default_link_set,
        help="Path to the MGeo link_set.json file.",
    )
    parser.add_argument(
        "--mgeo-dir",
        default="",
        help="MGeo directory containing link_set.json. Used when --link-set is not explicitly changed.",
    )
    parser.add_argument(
        "--output-csv",
        "--output-file",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Optional waypoint spacing in meters. <=0 keeps original link points.",
    )
    parser.add_argument(
    ...
    if args.mgeo_dir and args.link_set == default_link_set:
        link_set_path = Path(args.mgeo_dir) / "link_set.json"
    else:
    ...
    points = resample_points(points, args.resolution)
