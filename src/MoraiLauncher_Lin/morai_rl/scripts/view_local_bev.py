...
def build_renderer(config_path: str) -> tuple[object, LocalBeVRenderer, str, str]:
    ...
    return receiver, renderer, transport_name, transport_detail
...
def main() -> None:
    ...
    args = parse_args()
    receiver, renderer, transport_name, transport_detail = build_renderer(args.config)
    receiver.start()
    ...
    try:
        ...
        while running:
            ...
            if state is None:
                waiting = font.render(
                    f"Waiting for MORAI vehicle status {transport_name}...",
                    True,
                    (230, 230, 230),
                )
                hint = small_font.render(
                    f"Listening on {transport_detail}",
                    True,
                    (180, 180, 180),
                )
