...
- MORAI ROS Synchronous Mode용 adapter (`stage1_ros_sync_config.toml`)
...
- `stage1_ros_sync_config.toml`
  ROS bridge / Synchronous Mode용 RL8 기반 설정
...
ROS Synchronous Mode를 사용할 때는 catkin setup의 Python 경로를 유지한 채 `morai_rl` 경로를 앞에 붙입니다.
...
### ROS Synchronous Mode

MORAI ROS bridge가 실행 중이고 Sync 관련 service가 보이는 상태에서 아래 설정을 사용합니다.

```bash
python -m morai_rl.scripts.check_step_loop \
  --config morai_rl/stage1_ros_sync_config.toml \
  --steps 50
```

학습은 기존 PPO 스크립트를 같은 설정으로 실행합니다.
