...
class MoraiRLEnv:
    ...
    def reset(self) -> tuple[object, dict]:
        ...
        for attempt_index in range(max(1, self.config.reset.max_reset_attempts)):
            if not (self.config.ros.enabled and self.config.ros.use_sync_mode):
    ...
    def step(
        ...
        last_state_version = (
            self.vehicle_receiver.get_version()
            if self.config.ros.enabled
            and self.config.ros.use_sync_mode
            and hasattr(self.vehicle_receiver, "get_version")
            else None
        )
        ...
        state = self._wait_for_latest_state(
            min_timestamp_sec=None if last_state_version is not None else last_timestamp,
            min_version=last_state_version,
        )
    ...
    def _send_zero_for(self, duration_sec: float, allow_create: bool = True) -> None:
        ...
        while time.monotonic() < end_time:
            try:
                if self.config.ros.enabled:
                    self.control_client.send(ControlCommand.zero(), set_gear=False)
                else:
    ...
    @staticmethod
    def _coerce_action(action: ControlCommand | tuple[float, ...] | list[float]) -> ControlCommand:
